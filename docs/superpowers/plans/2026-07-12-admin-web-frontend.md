...
**참고 문서/파일 (읽어야 할 것):**
- Spec: "배포" 섹션
- `docker-compose.yml` — 기존 서비스 구조, `gongu-net` 네트워크명, `server` 서비스의 `CORS_ALLOWED_ORIGINS` 환경변수, `SERVER_PORT` 호스트 포트 매핑
- `admin-web/.env.example` — Task 2에서 만든 `VITE_API_BASE_URL` 예시값
