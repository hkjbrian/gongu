...
`portone.webhook-secret`이 비어 있으면 모든 웹훅을 거부한다(빈이 생성될 때 ERROR 로그 1회,
이후 요청마다 WARN). `PORTONE_WEBHOOK_SECRET` 환경변수로 주입하며 저장소에 실제 값을 두지 않는다.
운영 환경에서 `PORTONE_WEBHOOK_SECRET`은 필수다 — 주입되지 않으면 엔드포인트가 모든 웹훅을
거부하고(fail-closed), 시크릿이 제공될 때까지 서버 사이드 확정 백스톱을 잃는다.
perf 프로파일은 웹훅 서명 경로를 검증하지 않는다.

## 범위 밖

- `webhook-id` 기반 중복 수신 저장/차단 — 타임스탬프 검사(±300초)는 **freshness 검사**로,
  재전송 위험을 300초 창으로 좁힐 뿐 완전히 막지는 못한다. 창 안에서 캡처된 유효 웹훅을
  재전송하면 `PENDING` 상태의 `completePayment`가 PG 조회를 반복할 수 있다(`PAID`의 멱등
  처리만으로는 차단되지 않음). 완전 차단은 `webhook-id`를 원자적으로 저장해 중복을 거르는
  작업이 필요하며, 별도 이슈(#219)로 추적한다. TLS + 300초 제약으로 실사용 위험은 낮다.
- 공식 SDK 마이그레이션.
