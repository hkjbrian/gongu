...
[1] 리뷰 실행 — fresh 서브에이전트에게 코드 리뷰 위임, 또는 CodeRabbit 등 외부 자동 리뷰가 이미 PR에 달린 경우 그 결과 활용 → 파일경로:라인번호 형태로 이슈 출력
[2] Claude가 리뷰 결과를 GitHub 인라인 코멘트로 포스팅 (외부 리뷰가 이미 인라인으로 달려 있으면 생략)
[3] Claude가 모든 코멘트를 수집하여 이슈별 판정 수행 (판정은 반드시 구현에 참여하지 않은 fresh 서브에이전트에게 위임)
[4] Claude가 판정 요약을 사용자에게 제시 → 합의
[5] 합의 후 각 thread에 판정 reply → 서브에이전트에 수정 위임 → 리뷰 재실행
...
### 리뷰 컨텍스트 (리뷰 서브에이전트 프롬프트에 직접 명시)

Codex CLI와 달리 Claude 서브에이전트는 `AGENTS.md`를 자동으로 읽지 않으므로, 리뷰를 위임할 때 아래 문서를 프롬프트에 직접 포함시킨다:
...
fresh 서브에이전트(이번 구현에 참여하지 않은)에게 `code-review` 스킬 또는 `requesting-code-review`의 코드 리뷰어 프롬프트로 위임한다. PR에 CodeRabbit 등 외부 자동 리뷰가 이미 달려 있다면 그 결과를 그대로 활용해도 된다 — 이 경우 이 단계는 생략.

변경된 파일을 분석해 이슈를 `파일경로:라인번호` 형태로 출력받는다.

---

> ⛔ **STOP — [1] → [2]**
> 리뷰 결과가 나왔다고 바로 판정하지 않는다.
...
리뷰어(서브에이전트 또는 외부 자동 리뷰)가 매긴 severity를 판정 요약 표시로 변환한다. CodeRabbit 등 외부 리뷰는 자체 등급(Critical/Major/Minor 등)을 쓰는 경우 문맥에 맞춰 아래 표에 대응시킨다.

| 원 severity | 판정 요약 표시 | 의미 |
|-------|-------------|------|
...
2. 수정 사항 구현 (서브에이전트에게 다시 위임한다 — superpowers:subagent-driven-development)
3. `./gradlew compileJava` 및 관련 테스트 통과 확인
4. 커밋: `fix: {수정 내용} (#이슈번호)`
5. push
6. [1]의 리뷰 실행을 변경분에 대해 재실행 (fresh 서브에이전트 재위임, 또는 외부 자동 리뷰의 재스캔 대기)
...
- **성능·구조 관련 지적**은 반드시 대안 탐색 후 선택지를 제시한다. 리뷰어가 제안한 방법을 그대로 수용하지 않는다.
