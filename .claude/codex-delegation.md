# 서브에이전트 위임 규칙

> 파일명은 `codex-delegation.md`로 남아 있지만, 2026-09-17부로 구현·리뷰 위임 대상은 Codex CLI가 아니라 Claude 서브에이전트다(CLAUDE.md 역할 분리 참고). 아래 규칙은 위임 대상이 무엇이든 동일하게 적용된다.

## 위임 전 Claude가 먼저 확인·결정할 것

엔티티/리포지토리 작업이 포함된 경우:
- `docs/schema/ddl.sql`을 직접 읽어 테이블명·컬럼명·NULL 여부·인덱스를 파악한다.
- 소프트 딜리트 불변식을 결정하고 프롬프트에 명시한다.  
  예) "`deletedAt IS NULL`이 삭제 여부의 권위 있는 신호. `isActive`는 서비스 레벨 비활성화 전용."
- 비즈니스 정책이 필요한 사항(이메일 재사용 허용 여부, 멱등성 요구사항 등)은 위임 전에 결정한다.

## 위임 시 프롬프트에 반드시 포함할 것

1. 구현할 파일의 정확한 경로
2. 관련 기존 파일 경로 (읽어서 참고하도록)
3. **엔티티 포함 시**: `docs/schema/ddl.sql` 경로를 명시하고 테이블명·컬럼명을 DDL 기준으로 맞추도록 지시
4. 적용해야 할 ADR/컨벤션 기준
5. 소프트 딜리트 불변식 및 결정된 비즈니스 정책
6. 금지 사항 (기존 파일 수정 금지 등)
7. **테스트 작성 지시** (아래 기준 참고)

## 테스트 작성 기준

모든 구현 위임에 테스트가 포함되어야 한다. 위임받는 서브에이전트에게 아래를 명시한다.

### 기본 (항상)
- **Service/Domain 단위 테스트** 필수
  - 정상 케이스 + 주요 예외 케이스 (도메인 규칙 위반, 존재하지 않는 엔티티 등)
  - 테스트 파일 경로를 프롬프트에 명시할 것

### 추가 (상황별)
| 상황 | 테스트 종류 |
|------|-----------|
| 커스텀 JPQL·네이티브 쿼리 작성 시 | `@DataJpaTest` |
| N+1 해결 방식 검증 시 | `@DataJpaTest` |
| Security/인가 로직 변경 시 | `@WebMvcTest` |
| DTO validation 추가 시 | `@WebMvcTest` |
| 결제·주문 등 다중 도메인 핵심 플로우 | `@SpringBootTest` |
| 멱등성 검증 시 | `@SpringBootTest` |

## 위임하지 않는 것

- GitHub 관리 (Issue/PR/Milestone): Claude가 직접 처리
- 커밋/push: Claude가 직접 처리
- 설계 결정 및 ADR 작성: Claude가 직접 처리
